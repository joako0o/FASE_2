...
### 9.30 Diagnóstico del deterioro con +59: lectura y márgenes, no nueva variante

El investigador solicita revisar los nuevos errores y pensar por qué falla. Se leyeron las16intervenciones cambiadas (57.722caracteres):9errores nuevos/3corregidos/4errores de otro tipo. Informe `docs/DIAGNOSTICO_AMPLIACION_TFIDF_59_V1.md`; protocolo antes de reconstruir en `docs/PROTOCOLO_DIAGNOSTICO_AMPLIACION_TFIDF.md`.

Se reconstruyeron exactamente las mismas condiciones control/ampliado del ensayo, con A/B/final idénticos en793casos por condición. Márgenes clase ampliada−clase control, 12.005contribuciones, interceptos separados y descomposición simétrica algebraica verificada. 48vecinos nuevos restringidos al train permitido, no influencias causales. Vocabularios/balanced y df de términos auditados. Replay de salidas byte a byte y116tests/0omitidos,16citas literales. No nuevas métricas de rendimiento, modelos guardados o cambios de etiquetas.

Hallazgos: preguntas semejantes a adhesión; nombres/cargos/fórmulas pueden pesar en dirección equivocada; recortar rechazado y sesgo retirado se confunden con adhesión; narrativa internacional/posibilidades se parecen a recomendaciones. Se señalaron fronteras de referencias en mantención/dirección implícita sin adjudicar alternativas, incluido un acierto nuevo contra v2. En8/9errores nuevos el componente de coeficientes es el mayor en magnitud; representación se opone al cambio en7/9. Es una descomposición convencional, no porcentaje causal ni culpabilidad individual.

Se recomienda uniformar la aplicación de reglas y buscar contrastes reales, no N al azar ni más H/D únicamente por cuota. No meter los16casos en su propio train, cambiar referencias para recuperarF1 o lanzar otra arquitectura/parámetro automáticamente. Meta300/300 y colaG siguen pendientes. La limpieza extrema final y los sintéticos continúan sin ejecutarse.


### 9.31 Opinión sobre calibración de un plan B histórico, sin ejecución

El investigador trae ejemplos de actas pre2000 (1977/1982/1986/1992), con etiquetas tentativas, y pide evaluar la idea antes de comenzar. Nota en `docs/CALIBRACION_PLAN_B_ACTAS_HISTORICAS.md`; no se copia/importa el corpus ni se entrenan modelos, etiquetan definitivamente filas o cambian reglas del proyecto actual.

La calibración mezcla alivio financiero/regulatorio con postura monetaria: multa30→20 no es tasa de política (78); préstamos/prórrogas a Bolivia no son D chilena (4480/4481); ayudas focalizadas no prueban relajación agregada (26/57/etc.); una excepción a prohibición no es endurecimiento por sí sola (42). N/0 de rutina tampoco se presume cuando hay una norma no identificada5→10 (18566). Ratificación y ayuda deben someterse al mismo criterio de función/alcance, no palabras clave. Varias filas mezclan varios acuerdos o están cortadas.

Propuesta, pendiente: unidad por asunto/discusión-resolución; campos de función/instrumento/alcance y dirección identificable; fuera de alcance/contexto insuficiente distinto de N sustantivo; codebook histórico separado si se amplía el objetivo. Conservar fuentes, no reconstruir faltantes ni usar información omitida como etiqueta que el modelo no puede aprender. Fuentes oficiales consultadas sobre cambios de régimen/instrumento (flotación1999, nominalización2001); no equivalen a auditoría de las actas antiguas. Plan B separado, no para completar cuotas300 ni sustituir validación intacta. Cualquier prueba de transferencia requiere autorización/diseño posterior.


### 9.32 Propuestas de definición y filtro histórico de tasa, no aplicadas

El investigador pidió definir qué es/no es postura y consultar otros trabajos. Se documentó la búsqueda dirigida en `docs/DEFINICION_POSTURA_MONETARIA_PROPUESTA_V1.md` (Shah/WCB/Ornithologist, FMI1991/1995, BIS/ECB). Aporte clave: función antes de instrumento; el crédito dirigido o refinanciamiento histórico no se excluye solo por ser sectorial, pero ayuda no demuestraD por sí sola. No es un nuevo codebook aprobado ni se migran etiquetas.
