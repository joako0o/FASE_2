...
**Estado operativo vigente:** selección de 60 candidatos reales ya creada y entregada en `data/auditoria/ampliacion_hd_60_v1/revision_60_candidatos.xlsx`, seis bloques de diez. **Esperar devolución humana, incluso parcial; no regenerar ni sobrescribir.** No hay etiquetas nuevas aceptadas ni ampliación de train. Protocolo `docs/AMPLIACION_HD_60_V1.md`; revisar manifiesto/textos/citas/pendientes/ayuda cuando vuelva el libro. No aplicar el importador de las 30 anotaciones anteriores. El archivo técnico `NO_CONSULTAR_antes_de_responder_seleccion.json` contiene pistas de búsqueda; no exponerlas al investigador antes de su decisión.

106 pruebas aprobadas, 0 omitidas; selección reproducida y 202.789 caracteres del XLSX verificados contra L0. 56 reuniones, 29 actores; excluidos anotados y marco humano por IDs/copias; folds potenciales/prohibidos por reunión/texto registrados, no ejecutados. Libro sin etiquetas sugeridas ni hojas ocultas. La muestra es dirigida para desarrollo, no test. Sin sintéticos, entrenamiento ni cambios al paquete BETO.
