...
**Estado operativo vigente:** el usuario aclaró que el agente debe buscar y puntuar los ejemplos para entrenamiento. **No esperar que complete el Excel de 60 candidatos.** La primera tanda IA C01–C30 está anotada en `data/auditoria/ampliacion_hd_60_v1/anotacion_ia_v1/tanda_01/`: 7 H, 13 D, 10 N; 18 H/D de confianza alta (6 H + 12 D), dos H/D medios en reserva y una duda N excluida. Citas literales y textos íntegros verificados. Leer `docs/ANOTACION_IA_AMPLIACION_HD_V1.md`.

**Siguiente acción: anotar C31–C60 por lectura completa**, en otra tanda bajo el presupuesto de 20.000 palabras. No son respuestas humanas ni gold. No usar las pistas de búsqueda como etiquetas, forzar H/D o importar pendientes. Los nuevos archivos permanecen fuera de `data/etiquetas/*.csv` y no cambian el paquete activo. Hay plan de purga por fold, pero aún no train ampliado. No entrenar con esos JSON como si fueran un paquete de 38/39. Antes de nuevas selecciones, excluir también los IDs de la capa IA, no solo los 1.352 originales.

El Excel de selección queda como artefacto histórico/opcional, sin respuestas humanas recibidas y sin rellenarlo en nombre del usuario. La selección original pasó 106 pruebas de software; la tanda nueva tiene controles de citas, hashes, exclusiones y planes, no segunda anotación semántica independiente. 30 textos leídos: 107.796 caracteres / 17.674 palabras. No se agregaron scripts ni se entrenó en esta tanda.
