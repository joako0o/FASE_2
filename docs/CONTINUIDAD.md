...
Lotes cerrados: prioritarias **77/77** (11cambios de etiqueta+1relevancia); pilotos r1 **110/110** (3cambios), r2 **88/88** (1), r3 **85/85** (0), r4 **17/17** (0); escalados r5 **99/99** (2), r6 **93/93** (5), r7 **103/103** (5), r8 **52/52** (8), r9 **94/94** (4), r10 **120/120** (7) y r11–r17 **414/414** (8). Acumulado **1.352/1.747**, quedan395; las 19 rondas IA originales están cerradas. Leer los informes `docs/REVISION_*_V3.md`. Continuar con las 89 IA nuevas y luego las 306 humanas, revisando falsos N y sin presentar recodificación como mejora del modelo.
...
Siguiente recomendado: acordar cómo resolver convenciones relativas frente a dirección respaldada, priorizar las77filas y extender revisión por familias/control, incluyendo nuevas89bajo criterio uniforme. No reclasificación masiva automática ni tocar el gold; si se aprueba una referencia nueva, preservar la antigua y separar recodificación de mejora del modelo. Pausar nuevas incorporaciones/entrenamiento mientras se resuelve esta compatibilidad, sin cancelar la meta300/300.

**Aclaración importante del investigador:** pregunta si las anotaciones originales IA/humanas quedan mal según el nuevo criterio, no solo las89nuevas. No concluir invalidez global ni compatibilidad total: no se ha auditado toda la colección bajo otra definición. Distinguir aplicación de regla de cambio de variable; preservar1.352IA/306humanas y no reabrir306por esta consulta. La tercera salida histórica mejoró relevanciaN, pero mantiene pendientes de criterio/citas; ver sección final de `docs/CRITERIO_PLAN_B_TASA_POLITICA_PROPUESTA.md`. La cita21716tiene291caracteres, no excede300. No se importó la salida ni se reclasificó/entrenó.

**Última instrucción atendida:** definiciones destacadas en README y `docs/REGLAS.md` §13, especialmente N monetario relevante=1 frente a fuera de alcance/administrativo=0. El investigador pregunta si reclasificar lo anterior; se recomienda auditar consistencia (primero89nuevas, luego familias de riesgo y controles de la base1.352), no modificar todas las etiquetas ni solo errores del modelo. **No se inició esa auditoría ni se aprobó/ejecutó migración automática a otro codebook.** No conocemos aún el número de cambios. Preservar v2/resultados y decisiones humanas; cambios por ID solo tras revisión/versionado, sin abrir306respuestas ni confundir mejora de referencia con mejora de modelo.

**Última instrucción:** antes de seguir con plan A, el investigador pide criterio y calibración de15nuevos fragmentos pre2000 centrados en tasas. Leer `docs/CRITERIO_PLAN_B_TASA_POLITICA_PROPUESTA.md`. No se importa/entrena. Relevancia ≠ dirección; opciones y descripción monetaria N conservan relevancia1; no convertir todo N en0. No usar H/D como ranking de magnitudes/oposición a la mayoría. Historia no implica postura actual; aperturas pueden contener decisiones sustantivas; correcciones de texto exigen cotejo.

Borrador general de investigación en `docs/DEFINICION_POSTURA_MONETARIA_PROPUESTA_V1.md`, también propuesto/no aprobado: distingue función de instrumentos históricos y evita excluir automáticamente crédito dirigido/refinanciación. **No sustituye v2 ni autoriza reclasificar casos existentes.** La guía acotada a tasa de política es la recomendación más reciente para el plan B; aún se debe acordar antes de incorporar sus etiquetas. No se avanzó en nuevas anotaciones/entrenamientos del plan A mientras se responde esta consulta.
...
- Las antiguas 306 anotaciones no son un test independiente intacto. Para v3 está autorizado abrir y revisar sus respuestas, preservando v2 y sin representarlas después como test ciego intacto. Fecha humana confirmada: 2026-09-16. Las 30 anotaciones posteriores ya fueron devueltas en XLSX; no pedirlas otra vez.
