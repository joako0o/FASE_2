...
**Última decisión y preparación:** el investigador aprobó el `codebook_v3.md`, basado en dirección monetaria respaldada y no postura relativa al menú/fase, y autorizó revisar **IA + humanas**. Universo inventariado: 19 rondas IA/1.352 IDs, cuatro tandas IA nuevas/89 y humanas v2/306; **1.747 IDs distintos en 24 lotes**. Las humanas dejan de ser test ciego para v3, pero su versión v2 se preserva. Inventario reproducible en `data/auditoria/migracion_v3/`, comando 40 `preparar-migracion-v3`; 10 pruebas específicas aprobadas. No se entrenó ni sobrescribió origen.

Las dos rondas prioritarias r13_tanda09/r14_tanda10 están **cerradas 77/77** contra v2 vigente:65compatibles,11cambios de etiqueta (D→N5, D→H2, H→N2, H→D1, N→D1) y un cambio de relevancia0→1; cero pendientes. La pantalla inicial de29H/D queda superada por `docs/REVISION_RONDAS_PRIORITARIAS_V3.md`, que además evita volver a contar dos correcciones ya presentes en v2. Quedan1.670 IDs. Continuar por lotes, revisando también falsos N; registrar cita/fundamento/confianza/hash y no presentar recodificación como mejora del modelo.
