...
def run(out=OUT):
    ...
    sentence_df = np.asarray(sentence_binary.sum(axis=0)).ravel(); candidate_ks=[6,8,10,12,14,16,18]; benchmark_models={}; benchmark_rows=[]; benchmark_topics=[]
    ...
    k6_metrics=benchmark[benchmark.k.eq(6)].iloc[0]; k14_metrics=benchmark[benchmark.k.eq(14)].iloc[0]; k18_metrics=benchmark[benchmark.k.eq(18)].iloc[0]
    save_json(out / "segmentacion_nmf_seleccion.json", {"k_evaluados":candidate_ks,"k_seleccionado":14,"k6_probado":True,"conclusion_k6":"Alta coherencia y estabilidad, pero un componente concentra cerca de 46% y solo tres de seis componentes son dominios económicos sustantivos; los demás capturan estructura/decisión. No se recomienda usar sus seis componentes como seis habilidades FIFA.","razon_k14":"Mantiene estabilidad prácticamente perfecta y diversidad alta, reduce la prevalencia máxima a cerca de 15%, y es el primer candidato que separa actividad, trabajo, inflación, commodities, tipo de cambio y tasas sin las particiones más redundantes de K=16/18.","comparacion_clave":{"k6":{"coherencia_npmi":float(k6_metrics.coherencia_npmi_top10),"prevalencia_maxima":float(k6_metrics.prevalencia_maxima),"estabilidad":float(k6_metrics.estabilidad_media)},"k14":{"coherencia_npmi":float(k14_metrics.coherencia_npmi_top10),"prevalencia_maxima":float(k14_metrics.prevalencia_maxima),"estabilidad":float(k14_metrics.estabilidad_media)},"k18":{"coherencia_npmi":float(k18_metrics.coherencia_npmi_top10),"diversidad":float(k18_metrics.diversidad_top10),"estabilidad":float(k18_metrics.estabilidad_media)}},"unidad_seleccionada":"oracion_min_80","razon_unidad":"Intervención y actor-reunión mezclan contenido económico con fórmulas procedimentales; la oración separa mejor dominios semánticos. La asignación final se proyecta luego a cada intervención.","criterio":"Selección multicriterio: coherencia, estabilidad, diversidad, redundancia, balance de prevalencia e interpretación; el error de reconstrucción no se usa solo porque siempre mejora al aumentar K."})
    ...
    radar_axes = {
        "actividad_demanda":["tema_nmf_01"], "mercado_laboral":["tema_nmf_02"], "inflacion_expectativas":["tema_nmf_09"],
        "entorno_externo_commodities":["tema_nmf_03","tema_nmf_08","tema_nmf_13"], "tipo_cambio":["tema_nmf_10"], "mercados_estructura_tasas":["tema_nmf_12"]}
