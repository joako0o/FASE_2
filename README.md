...
La restricción es sustantiva: la decisión proxy es la última fila institucional `Consejo del Banco Central de Chile` clasificada en cada reunión (132/132; 107 N, 18 H y 7 D), no un voto observado. Para reemplazarla progresivamente se construyó `base_votos_acta_actor.csv`, con **649 pares reunión–consejero**: 516 acciones se extrajeron de formulaciones explícitas de voto, 87 se infirieron separadamente porque el acta declara unanimidad y 46 quedaron como `no_extraido`. Los 132 acuerdos institucionales sí tienen acción y TPM objetivo: 80 mantenciones, 35 alzas y 17 bajas. Hay ocho votos explícitos cuya acción difiere del acuerdo, candidatos naturales a disenso. Toda extracción automática conserva el fragmento y texto fuente y continúa marcada como no validada hasta su auditoría humana; `voto_accion` nunca mezcla inferencias, mientras `voto_accion_con_inferencia` las incorpora y `fuente_voto` permite separarlas. Las tendencias anuales tampoco separan inclinación personal del ciclo macroeconómico o del cambio en composición temática; `variacion_anual_por_actor.csv` es descriptivo, no la estimación de $\alpha_i$ y $\beta_i$.
